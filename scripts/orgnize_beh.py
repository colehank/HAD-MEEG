...
runs = [f"{run:0>2d}" for run in np.arange(1, 9)]
...
def compute_beh(p_mat):
    ...
    truth_label = [
        class_info["sports_label"][class_info["className"] == name[0][0]].iloc[0]
...
def generate_meta_data(p_mat):
    ...
    resp_is_right = [stim == resp for stim, resp in zip(stim_is_sports, resp_is_sports)]
    videos = data["runStim"]
    videos_idx = [
