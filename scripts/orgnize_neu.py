...
all_subinfo = pd.read_excel(
    '/nfs/z1/userhome/zzl-zhangguohao/workingdir/BIN/action/action原始编号.xlsx',
)
...
def write_dst_bids(raw, dst_bids):
    ...
    if dtype == 'meg':
        empty_raw = find_empty_room_raw(
            raw.info['meas_date'].strftime('%Y%m%d'),
        )
        if empty_raw is None:
            tqdm.write(
                f"⚠️ No matching empty room for date {raw.info['meas_date'].strftime('%Y%m%d')}",
            )
