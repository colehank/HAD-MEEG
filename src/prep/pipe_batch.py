...
class BatchPrepPipeline:
    ...
    def _run_one(
        ...
        deriv_raw = Path(deri_root) / 'preproc' / f'sub-{bids.subject}' / f'ses-{bids.session}' / bids.datatype / f'{bids.basename}_preproc_{bids.datatype}.fif'
        deriv_ica = Path(deri_root) / 'preproc' / f'sub-{bids.subject}' / f'ses-{bids.session}' / bids.datatype / f'{bids.basename}_desc-ica_{bids.datatype}.fif'
