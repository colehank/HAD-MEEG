...
def component_(ica, comp):
    return ica.get_components()[:, comp]
...
def make_main_plot(
    ...
    for ax, (name, comp) in zip([ax1, ax2, ax3], arti_data.items()):
        ...
        ax.set_title(name, va="center", ha="right", rotation=90, x=0.2, y=0.3)
        ax.set_adjustable("datalim")
        # pos = ax.get_position()
        # new_pos = [0.16, pos.y0-0.05, pos.width, pos.height]
        # ax.set_position(new_pos)
        # ax.text(1.05, 0.5, name, va='center', ha='left', transform=ax.transAxes, rotation=90, fontsize=fontsize)

    # ICA sources
    for artifact, ax in zip(arti_data.keys(), [ax4, ax5, ax6]):
        ica_artifact = sources_(ica, raw4plot, arti_data[artifact], tlim=(T_MIN, T_MAX))
        ax.plot(ica_artifact[1] - T_MIN, ica_artifact[0], color="black", alpha=0.7)
    ...
    offset = 10
    axes = [ax7, ax8]
    for meg_data, eeg_data, ax in zip(
        [pre_data, pos_data], [Epre_data, Epos_data], axes
    ):
        # MEG
        for i in range(len(meg_data[0])):
            ax.plot(
                meg_data[1] - T_MIN,
                meg_data[0][i] + i * offset,
                color=colors[1] if ax == ax7 else colors[2],
                alpha=0.85,
            )
            ax.text(
                meg_data[1][-1] - T_MIN,
                meg_data[0][i][-1] + i * offset + 3,
                f"MEG-{picks_ch[i].split('-')[0]}",
                va="center",
                ha="right",
                color="black",
                fontsize=fontsize - 4,
        ...
        for i in range(len(eeg_data[0])):
            ax.plot(
                eeg_data[1] - T_MIN,
                eeg_data[0][i] + eeg_offset + i * offset,
                color=colors[1] if ax == ax7 else colors[2],
                alpha=0.85,
            )
            ax.text(
                eeg_data[1][-1] - T_MIN,
                eeg_data[0][i][-1] + eeg_offset + i * offset + 4,
                f"EEG-{Epicks_ch[i]}",
                va="center",
                ha="right",
                color="black",
